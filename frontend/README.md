...
| 构建工具 | Vite **7**（devDependencies `^7.0.0`）|
...
| **计划卡片** | 复杂任务先渲染 LLM 生成的执行计划（`plan` 事件，含步骤/要点/并行分组），子代理执行后主 agent 汇总作答 |
...
| 功能 | 说明 |
|------|------|
| **长期记忆列表** | 显示 Java 向量记忆仓库中的持久化条目（重启不丢失），含重要性评分 |
...
| **自动分块** | 后端按段落/句子边界智能分块，写入 Java 向量记忆 |
...
| **规格文档编辑** | Markdown 编辑器 + 实时预览切换，内容同步至 Java 后端 |
...
| 功能 | 说明 |
|------|------|
| **服务健康检测** | 实时检测前端 / Java 后端连通状态（Python Agent 已退役）|
...
| **云端服务商管理** | 新增/编辑/删除 OpenAI-compatible 服务商（BaseURL + API Key + Model）；七家预设快填；一键激活切换云端/本地，配置持久化 |
...
项目数据存 `localStorage`（轻量，无需 IndexedDB）；规格文档和任务树同步至 Java `/api/project/*`。
...
开发时所有请求经 Vite 代理转发到 Java（8080）；Java-only 单后端。
...
| F-03 | clearMessages() 只清前端，不清服务端短期记忆 | ✅ 已修复（2026-05-30）|
