...
- **多飞书账户切换（暂不做）**：当前仅支持个人测试企业下的个人账户（单账户配置足够）。阶段 6 统一切到 PWA，届时飞书通道退为可选辅助，不需要在此阶段预建多账户切换机制。
- **阶段 6 探索项（暂不立项）**：个人测试企业账户 → 个人账户的跨账户切换。过渡期两者可并行（多账户 JSON 结构天然支持），但切换优先级逻辑等到阶段 6 灵魂迁移时一并设计，避免此刻过度工程化。
